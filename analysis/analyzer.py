...
class Analyzer:
    def __init__(self, will_train, args_dict):

        # If no model name/path is given, use mine/BERT depending on task.
        if args_dict.get("model_name_or_path") is None:
            if will_train:
                args_dict["model_name_or_path"] = "bert-base-uncased"
            else:
                args_dict["model_name_or_path"] = "barissayil/bert-sentiment-analysis-sst"

        # Set up configuration.
        self.config = AutoConfig.from_pretrained(args_dict["model_name_or_path"])
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args_dict["model_name_or_path"])
